...
contract AuctionHouse {
    ...
    function createAuctionItem(ItemData calldata _itemData) public returns (uint256) {
        if (
            _itemData.startingPrice == 0 || _itemData.duration_In_Hour == 0 || bytes(_itemData.name).length == 0
                || bytes(_itemData.description).length == 0
        ) revert AuctionHouse_MissingCreationFields();

        uint256 _newItemId = itemCount;
    ...
    function getCurrentHighestBid(uint256 _itemId) external view returns (uint256) {
        AuctionItem storage item = items[_itemId];
